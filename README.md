...
- Hard mode is an optional Middle mode variation: the same four credits remain visible as clues, but the answer must be typed as an actor or film title. Hard and Normal records are stored separately.
...
- First rounds receive an extra fairness pass: all four Middle options must be recognisable, have distinct titles and avoid sharing a major franchise label, while value-mode openers favour recognisable targets. This only affects future puzzles; frozen dates remain unchanged.
...
All player progress, streaks, scores and backup data are stored in browser `localStorage`. A service worker caches the app shell and frozen puzzle catalogue so the latest loaded line remains playable without a connection. The header's `GameGrid` link opens [GameGrid](https://harrydbarnes.github.io/GameGrid/). The default share output contains the puzzle number, date, Deviate score, placement accuracy, direction glyphs and site URL, never the anchor clues; the completion screen also offers an explicit answer-sharing option.
