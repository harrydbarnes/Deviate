...
- New hidden clues favour A/B-tier actors and recognised credits, distribute value targets away from a free midpoint strategy where the data allows it, and avoid reusing a clue or actor within the recent rolling window.
...
All player progress, streaks, scores and backup data are stored in browser `localStorage`. The share output contains only the date, Deviate score, placement accuracy, direction glyphs and site URL, never the anchor clues.
